...
function serveDesktopStatic(app2) {
...
var { app, BrowserWindow, shell } = require2("electron");
var mainWindow = null;
var server = null;
var __filename = fileURLToPath(import.meta.url);
var __dirname = path4.dirname(__filename);
function appRoot() {
  return app.isPackaged ? app.getAppPath() : path4.resolve(__dirname, "..");
}
function pegasusSupportDir() {
  return path4.join(app.getPath("appData"), "PegasusNEO");
}
function writeRuntimeLog(message, error) {
  try {
    const supportDir = pegasusSupportDir();
    fs4.mkdirSync(supportDir, { recursive: true });
    const detail = error instanceof Error ? `${error.stack || error.message}` : error === void 0 ? "" : String(error);
    fs4.appendFileSync(
      path4.join(supportDir, "desktop-runtime.log"),
      `[${(/* @__PURE__ */ new Date()).toISOString()}] ${message}${detail ? `
${detail}` : ""}
`
    );
  } catch {
...
async function startEmbeddedServer() {
  ...
  process.env.PEGASUS_STATIC_DIR = path4.join(root, "dist-client");
  process.env.NODE_ENV = "production";
  process.env.OWNER_OPEN_ID ||= "owner";
  process.env.OWNER_NAME ||= "Admin";
  process.env.LM_STUDIO_API_BASE ||= "http://127.0.0.1:1234/v1";
  const embeddedServer = await startServer(3847);
